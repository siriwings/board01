...
/* ARTICLE POST */
export function articlePostRequest(title,contents, publish) {
    return (dispatch) => {
        // inform MEMO POST API is starting
        dispatch(articlePost());

        return axios.post('/api/board/', {title,contents, publish}, {
 ...
 */
export function articleListRequest(pageNumber, articleCount) {
    return (dispatch) => {
        // inform memo list API is starting
        ...
        return axios.get('/api/board',{
            ...
            .then((response) => {
              ...
              //  console.log(response.data.count);
                dispatch(articleListSuccess(response.data.articles,response.data.count));
            }).catch((error) => {
...
//count: 전체글 개수
export function articleListSuccess(data,count) {
    return {
